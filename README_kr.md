...
이 프로젝트는 transformer 모델을 위한 sparse attention 패턴을 구현하고 시각화합니다. Sparse attention 패턴은 모델의 지역적 및 전역적 의존성 포착 능력을 유지하면서 계산 복잡도와 메모리 사용량을 크게 줄입니다. CIFAR-10(32x32 픽셀 = 1024 토큰)이 예시로 사용되지만, 구현은 일반적이며 다양한 시퀀스 길이와 작업에 적용할 수 있습니다.
